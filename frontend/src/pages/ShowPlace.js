...
export default function ShowPlace() {
    ...
    const [about , setAbout] = useState('ایدهٔ این‌که پروانه‌ای می‌تواند باعث تغییری آشوبی شود نخستین بار در ۱۹۵۲ در داستان کوتاهی به نام آوای تندر اثر ری بردبری مطرح شد. عبارت «اثر پروانه‌ای» هم در ۱۹۶۱ در پی مقاله‌ای از ادوارد لورنتس به وجود آمد. وی در صد و سی و نهمین ؟»')
  ...
  return (
    <div>
        <button className={style.btn} id={style.exit} onClick={exitHandler}>خروج</button>
        <div className={style.grid}>

            <div className={style.container} style={{backgroundImage: `url(${artwork})`}}>
            ...
            <div className={style.Name}>
                ...
                <input type='text' id={style.title} className={style.values} value={name}/>
            </div>
            
            <div className={style.Price}>
                <h2 id={style.title}>قیمت</h2>
                <input type='text' id={style.title} className={style.values} value={price}/>
            </div>
            ...
            <div className={style.likecontain}>
                <img src={like} className={liked ? style.liked : style.like} alt="" onClick={likeHandler}/>
                <input id={style.count} value={count}></input>
            </div>

            <img src={next} alt="next" id={style.next} onClick={nextHandler} />

            <button className={style.btn} id={style.buy} onClick={buyHandler}>خرید</button>
            <button className={style.btn} id={style.play} onClick={playHandler}>پخش/توقف</button>
